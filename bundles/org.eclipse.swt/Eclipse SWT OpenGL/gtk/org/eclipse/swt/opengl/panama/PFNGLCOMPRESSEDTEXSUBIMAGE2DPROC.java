...
public interface PFNGLCOMPRESSEDTEXSUBIMAGE2DPROC {
    ...
    static NativeSymbol allocate(PFNGLCOMPRESSEDTEXSUBIMAGE2DPROC fi, ResourceScope scope) {
    ...
    static PFNGLCOMPRESSEDTEXSUBIMAGE2DPROC ofAddress(MemoryAddress addr, ResourceScope scope) {
        NativeSymbol symbol = NativeSymbol.ofAddress("PFNGLCOMPRESSEDTEXSUBIMAGE2DPROC::" + Long.toHexString(addr.toRawLongValue()), addr, scope);
return (int x0, int x1, int x2, int x3, int x4, int x5, int x6, int x7, jdk.incubator.foreign.MemoryAddress x8) -> {
            try {
                constants$73.PFNGLCOMPRESSEDTEXSUBIMAGE2DPROC$MH.invokeExact(symbol, x0, x1, x2, x3, x4, x5, x6, x7, (jdk.incubator.foreign.Addressable)x8);
            } catch (Throwable ex$) {
