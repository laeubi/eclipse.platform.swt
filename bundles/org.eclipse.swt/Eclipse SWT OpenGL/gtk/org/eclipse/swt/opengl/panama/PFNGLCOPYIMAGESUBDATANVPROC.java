...
public interface PFNGLCOPYIMAGESUBDATANVPROC {
    ...
    static NativeSymbol allocate(PFNGLCOPYIMAGESUBDATANVPROC fi, ResourceScope scope) {
    ...
    static PFNGLCOPYIMAGESUBDATANVPROC ofAddress(MemoryAddress addr, ResourceScope scope) {
        NativeSymbol symbol = NativeSymbol.ofAddress("PFNGLCOPYIMAGESUBDATANVPROC::" + Long.toHexString(addr.toRawLongValue()), addr, scope);
return (int x0, int x1, int x2, int x3, int x4, int x5, int x6, int x7, int x8, int x9, int x10, int x11, int x12, int x13, int x14) -> {
            try {
                constants$736.PFNGLCOPYIMAGESUBDATANVPROC$MH.invokeExact(symbol, x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14);
            } catch (Throwable ex$) {
