...
public interface GLDEBUGPROC {
    ...
    static NativeSymbol allocate(GLDEBUGPROC fi, ResourceScope scope) {
    ...
    static GLDEBUGPROC ofAddress(MemoryAddress addr, ResourceScope scope) {
        NativeSymbol symbol = NativeSymbol.ofAddress("GLDEBUGPROC::" + Long.toHexString(addr.toRawLongValue()), addr, scope);
return (int x0, int x1, int x2, int x3, int x4, jdk.incubator.foreign.MemoryAddress x5, jdk.incubator.foreign.MemoryAddress x6) -> {
            try {
                constants$253.GLDEBUGPROC$MH.invokeExact(symbol, x0, x1, x2, x3, x4, (jdk.incubator.foreign.Addressable)x5, (jdk.incubator.foreign.Addressable)x6);
            } catch (Throwable ex$) {
